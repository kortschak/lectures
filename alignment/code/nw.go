...
func makeTable(alpha alphabet.Alphabet, match, mismatch, gap int) NW {
	nw := make(NW, alpha.Len())
	for i := range nw {
...
func main() {
	...
	nwsa := &linear.Seq{Seq: alphabet.BytesToLetters([]byte("TGATCT"))}
	nwsa.Alpha = alpha
	nwsb := &linear.Seq{Seq: alphabet.BytesToLetters([]byte("AGATGCT"))}
	nwsb.Alpha = alpha

	needle := makeTable(alpha, 1, 0, 0) // (Match: 1, Mismatch: 0, Gap: 0)
