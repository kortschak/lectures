...
// SW is the linear gap penalty Needleman-Wunsch aligner type.
type SW Linear
...
func makeTable(alpha alphabet.Alphabet, match, mismatch, gap int) SW {
	nw := make(SW, alpha.Len())
	for i := range nw {
...
func main() {
	...
	swsa := &linear.Seq{Seq: alphabet.BytesToLetters([]byte("TGATCT"))}
	swsa.Alpha = alpha
	swsb := &linear.Seq{Seq: alphabet.BytesToLetters([]byte("AGATGCT"))}
	swsb.Alpha = alpha

	smith := makeTable(alpha, 1, 0, 0) // (Match: 1, Mismatch: 0, Gap: 0)
